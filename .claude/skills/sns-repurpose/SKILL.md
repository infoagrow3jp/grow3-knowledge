...
1. **親投稿**：手動改行で3行以内、かつ120字以内とする。端末上の自動折り返しは
   行数に含めない。収まらない場合は行を増やさず文を削る。完結させない。
   続きを読む理由を残す。
2. **ぶら下げ投稿**：2〜3本。本文の分割（要約の再分割ではなく、親で開いた論点の続き）。

- **1スレッド1論点**。1記事から複数スレッドを作る場合は、反応が生まれやすい断面を切り出す。
- 冒頭（親）は「場面」か「違和感」から入る（理論・定義から始めない）。
- 読者の状況を想起させる一般的な問いかけは許容する。ただし、素材に根拠がないのに
  「経営者からよく言われます」「こんな相談がありました」など、実在の発言・相談実績
  として表現してはならない。
  - OK例：任せたはずなのに、確認が増えていませんか。
  - NG例：経営者の方から「任せると不安になる」とよく相談されます
    （素材にその事実がない場合）。
