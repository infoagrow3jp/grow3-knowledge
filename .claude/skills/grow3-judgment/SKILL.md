...
有益な情報を増やすことより、中核メッセージを濁らせる情報を外すことを優先する。ただし、中核メッセージの「なぜ」を支える理論・説明は、分量だけを理由に削らない（削ると存在理由が弱くなるかを必ず問う）。隣接テーマは削除ではなく「1〜2枚／1〜2文で触れ、深掘りは別テーマと明言」に留める。参照素材の採否は、その情報が原典に存在するかではなく、成果物の中核メッセージを強めるかで判断する。原典にあるという理由だけで情報を追加しない。
