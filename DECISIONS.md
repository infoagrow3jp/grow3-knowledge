...
### DEC-014｜工程ゲートを越える前に停止・再承認し、同一ゲート内の密結合修正は過度に分割しない
- 日付：2026-07-18
- 状態：確定
- 対象：Grow3 Knowledge OS全体の作業進行（設計・実装・記録の工程区分）
- 注記：本決定はDEC-015より先に成立した進行判断を、リポジトリへ後日記録したものである。DEC番号は判断の論理順を表し、commit順とは一致しない。
- 決定：

  **1. 基本原則**
  - 作業開始時に、今回扱う工程ゲート、対象成果物、許可する変更範囲、完了条件、今回進まない次工程を明示する。
  - 1つの作業単位では、原則として事前に宣言した1つの工程ゲートだけを扱う。
  - 工程ゲート完了後は、完了報告を行って一度停止する。
  - 前工程の成果物を人間が確認しないまま、次工程へ自動的に進んではならない。

  **2. 工程ゲートを越える場合**
  - 次工程へ進む場合は、前工程の完了報告後に、別の作業単位として変更範囲と完了条件を改めて宣言する。
  - 原則として翌セッション以降に行う。ただし「翌セッション」は翌日まで待つことを意味しない。
  - 同日・同一チャットであっても、次をすべて満たせば、新しい作業単位として再開できる。
    - 前工程が明示的に停止している
    - 前工程の成果物を人間が確認している
    - 次工程への進行が明示的に承認されている
    - 次工程の対象、変更範囲、完了条件、停止点を改めて宣言している
  - CursorやAIが、前工程の完了を理由に次工程を自動実行してはならない。

  **3. 同一ゲートとして扱う条件**
  - 次をすべて満たす複数の修正は、同一ゲート内の密結合した修正群として扱い、無意味に分割しない。
    - 同じ成果物または同じ設計判断を対象としている
    - 共通の完了条件で採否を判断できる
    - 新しい責務層へ進まない
    - 新しい実データ、外部依存または実行環境を必要としない
    - 変更範囲を事前に限定できる
    - 1つの横断diffとして一貫してレビューできる
    - 分割すると文脈再構築や取りこぼしのコストが増える

  **4. 新しい工程ゲートとする条件**
  - 次のいずれかに該当する場合は、新しい工程ゲートとして停止・再承認を必要とする。
    - 設計メモから実装へ進む
    - draftのレビューから凍結・DECISION確定へ進む
    - schema工程からvalidator工程へ進む
    - validatorのPhaseを越えて次Phaseへ進む
    - 架空fixture検証から実顧客データ検証へ進む
    - validatorからgenerator、skill、evalへ進む
    - 新しい責務層、依存ライブラリ、実行主体または出力契約を追加する
    - 事前宣言より変更対象や成果物の種類が広がる
    - 人間判断が必要な停止事項を実装判断として確定する
  - 「思考が乗っている」「軽そうに見える」ことだけを理由に、工程ゲートを越えてはならない。

  **5. 軽量タスク**
  - 実データや外部依存を必要とせず、相互に影響しない軽量タスクは、同一セッション内で連続して処理できる。
  - ただし、独立した目的を持つタスクは、作業範囲、差分確認、完了判定、commitを分離する。
  - 同一セッションで扱うことと、同一commitへ混在させることを同一視しない。

  **6. 判定基準**
  - 迷った場合は作業数ではなく、同じ成果物か、同じ責務層か、同じ完了条件か、同じ横断diffでレビューできるか、新しい人間判断を必要とするか、実データまたは外部依存が新たに必要かを基準とする。
  - 前4項目が同じで、後2項目が新たに発生しない場合は、同一ゲートとして扱える。
  - 新しい責務層、人間判断、実データまたは外部依存へ踏み込む場合は、新しい工程ゲートとして停止する。

- 理由：
  - 勢いは、密結合した論点を一貫して整理するための資産である。
  - 一方、勢いのまま次工程へ進むと、設計レビュー前の実装や変更範囲の拡大を招く。
  - 勢いそのものを止めるのではなく、工程ゲートを越える前に停止・再承認する。
  - 同一ゲート内を過度に分割すると、文脈再構築コストと取りこぼしが増える。
  - 作りすぎと刻みすぎの両方を防ぐ。
- 採用しなかった案：
  - 文字どおり1セッションにつき1作業へ限定する案。
  - 必ず翌日まで次工程へ進めない案。
  - 軽微な修正もすべて別セッションとする案。
  - 思考が乗っていれば工程ゲートを越えてよいとする案。
  - 同一セッションの変更をすべて同一commitへまとめる案。
- 見直し条件：
  - 同一ゲート判定が広く解釈され、作業範囲の拡大を止められない場合。
  - 過度な分割で文脈再構築コストや取りこぼしが増えた場合。
  - 実案件検証で最適な工程区分が異なると判明した場合。
  - Cursor、Codex等の実行環境に応じて「セッション」の意味が変わった場合。
